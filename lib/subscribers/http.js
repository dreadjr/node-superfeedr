...
Api.prototype.checkParameter = function (name, value) {
  var self = this;
...
Api.prototype.addFeed = function (topic, callback, secret, options) {
  ...
  return Promise.try(function () {
    ...
    assert.ok(topic.length <= 2048, 'topic');
    ...
    if (_.isString(callback)) {
      assert.ok(callback.length <= 250, callback);
    }

    if (_.isString(options.verify)) {
      assert.ok(this.checkParameter('verify', options.verify), 'options.verify');
    }

    if (_.isString(options.format)) {
      assert.ok(this.checkParameter('format', options.format), 'options.format');
    }
